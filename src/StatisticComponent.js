...
function StatisticComponent() {
    ...
    async function fetchUserStatusData() {
      try {
        ...
        await axios.post('http://localhost:4000/api/createUserStatus');
        
        // Fetch the generated user status data
        const response = await axios.get('http://localhost:4000/api/userStatus');
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                setUserName(userData.name); // Assuming the user's name is provided in the response
            } catch (error) {
    ...
    useEffect(() => {
        const fetchCategoryAttempt = async () => {
          try {
            const response = await fetch(`http://localhost:4000/api/user/${userId}/categoryAttempt?category=Basic Computer and Mobile Skill`);
            if (!response.ok) {
              throw new Error('Failed to fetch categoryAttempt data');
            }
            const data = await response.json();
            setCategoryAttempt1(data.categoryAttempt);
            setBestScore1(data.bestScore);
            setBestTime1(data.bestTime);
            setAveScore1(data.aveScore);
            setAveTime1(data.aveTime);
    ...
    const fetchCategoryAttempt = async () => {
        try {
        const response = await fetch(`http://localhost:4000/api/user/${userId}/categoryAttempt?category=Internet Skill`);
        if (!response.ok) {
            throw new Error('Failed to fetch categoryAttempt data');
        }
        const data = await response.json();
        setCategoryAttempt2(data.categoryAttempt);
        setBestScore2(data.bestScore);
        setBestTime2(data.bestTime);
        setAveScore2(data.aveScore);
        setAveTime2(data.aveTime);
    ...
    useEffect(() => {
        const fetchCategoryAttempt = async () => {
          try {
            const response = await fetch(`http://localhost:4000/api/user/${userId}/categoryAttempt?category=Communication Skill`);
            if (!response.ok) {
              throw new Error('Failed to fetch categoryAttempt data');
            }
            const data = await response.json();
            setCategoryAttempt3(data.categoryAttempt);
            setBestScore3(data.bestScore);
            setBestTime3(data.bestTime);
            setAveScore3(data.aveScore);
            setAveTime3(data.aveTime);
    ...
    const fetchCategoryAttempt = async () => {
        try {
        const response = await fetch(`http://localhost:4000/api/user/${userId}/categoryAttempt?category=Information Literacy Skill`);
        if (!response.ok) {
            throw new Error('Failed to fetch categoryAttempt data');
        }
        const data = await response.json();
        setCategoryAttempt4(data.categoryAttempt);
        setBestScore4(data.bestScore);
        setBestTime4(data.bestTime);
        setAveScore4(data.aveScore);
        setAveTime4(data.aveTime);
                ...
                return (
                    <div>
                        ...
                        <div className='grayBox2Stat'>
                            ...
                            <label className='label5'>BEST TIME</label>
                            ...
                            <label className='label7'>AVERAGE TIME</label>
                        ...
                        <div className='grayBox3Stat'>
                            ...
                            <label className='label9'>
                                A = Basic Computer and Mobile Skill&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;C = Communication Skill<br/> 
                            </label>
                            <label className='label10'>
                                B = Internet Skill&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&nbsp;&nbsp;D = Information Literacy Skill<br/> 
                            </label>
                            <div className='blueBox7Stat'>
                                <label className='label11'>The skill graph visualizes the strengths and weaknesses of the user in digital literacy, with a larger area indicating a broader skill set or higher proficiency.</label>
                            </div>
                        </div>
                    </div>
                );
